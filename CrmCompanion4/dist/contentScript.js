...
window.onload = function () {
    ...
    if (!top_CheckIfDynamicsUrl(urlString)) {
        if (allowLogging)
            console.log("app invalid for this page. Aborting");
        return;
    }
    if (allowLogging)
        console.log("Running");
...
function synchLocalAppstate() {
    if (allowLogging)
        console.log("in synchLocalAppstate");
    //default values
    _localAppstate = {
        menuIsOpen: false,
    ...
    chrome.storage.local.get("appState", (result) => {
        ...
        if (incomingAppState) {
            ...
            if (allowLogging)
                console.log("didn't receive any incoming appstate");
            callMenu();
        }
    });
}
function callMenu() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (iframeWrapper) {
            if (allowLogging)
                console.log("renewing properties instead");
        ...
        div.style.top = (_a = _localAppstate.menuYPosition + "px") !== null && _a !== void 0 ? _a : "40px";
        div.style.left = (_b = _localAppstate.menuXPosition + "px") !== null && _b !== void 0 ? _b : "40px";
        div.style.minWidth = "0px";
        div.style.width = "0px";
        div.style.minHeight = "0px";
        var fetchedHTML = yield (yield fetch(chrome.extension.getURL("customMenu.html"))).text();
        ...
        var dragimg = document.getElementById("CME_dragDivimg");
        dragimg.src = chrome.extension.getURL("svg_dragSymbol.svg");
        var closeimg = document.getElementById("CME_closeWindowimg");
        closeimg.src = chrome.extension.getURL("svg_closeSymbol.svg");
        var collapseimg = document.getElementById("CME_collapseimg");
...
function closeAndDeactivateMenu() {
    if (allowLogging)
        console.log("In closeAndDeactivateMenu");
    var iframeWrapper = document.getElementById("CME_iframeWrapper");
    if (!iframeWrapper) {
        return;
    }
    ...
    chrome.storage.local.set({ "appState": _localAppstate }, () => {
        if (allowLogging)
            console.log("appstates saved", _localAppstate);
        chrome.runtime.sendMessage({ text: "AppStateUpdate" });
    });
}
// Other functions 
function collapseAndUnfoldMenu() {
    if (allowLogging)
        console.log("In collapseAndUnfoldMenu");
    if (_localAppstate.menuIsCollapsed) {
        _localAppstate.menuIsCollapsed = false;
    }
    else if (!_localAppstate.menuIsCollapsed)
        _localAppstate.menuIsCollapsed = true;
    chrome.storage.local.set({ "appState": _localAppstate }, () => {
        if (allowLogging)
            console.log("appstates saved", _localAppstate);
        chrome.runtime.sendMessage({ text: "AppStateUpdate" });
